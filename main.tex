...
% TODO: Publish graph-transformation lib on maven central.
...
Formalizing \gls*{bpmn} would drastically reduce the cost of business process automation by facilitating the detection of errors and optimization potentials in process models already during design time.
...
General behavioral properties such as \textit{Safeness} and \textit{Soundness} were adapted to \gls*{bpmn} \cite{corradiniClassificationBPMNCollaborations2018}. They can uncover potential flaws in \gls*{bpmn} models leading to deadlocks or other undesirable execution states.

In this article, we consider two fundamental concepts when formalizing the execution semantics of \gls*{bpmn}.
First, \textit{state structure}, \ie, how model instances are represented during execution.
...
We define a \gls*{hot} from \gls*{bpmn} models to \gls*{gt} systems.
...
To define the execution semantics of a different behavioral language, one simply needs to establish a new execution metamodel and \gls*{hot} (see the language engineer in \autoref{fig:approach}).

% Contributions summary
The contribution of this article is twofold.
First, we introduce a new approach utilizing a \gls*{hot} to generate \gls*{gt} rules instead of providing fixed model-independent \gls*{gt} rules to formalize the semantics of a behavioral language.
...
We use typed attributed graphs to formalize the \gls*{bpmn} execution semantics.
...
\begin{figure}[ht]
  ...
  \caption{Example \gls*{gt} rules to start (left) and terminate (right) an activity}
...
The \gls*{gt} system for the order handling process containing both propositions can be found in \cite{timkrauterLMCS2024Artifacts2023}.
...
Our approach is implemented in a web-based tool called \textit{\gls*{bpmn} Analyzer}, which is open-source, publicly available, and does not require any installation \cite{timkrauterLMCS2024Artifacts2023, krauterFormalizationAnalysisBPMN2023}.
...
To evaluate the correctness of our \gls*{hot}, we have created a comprehensive test suite \cite{timkrauterLMCS2024Artifacts2023}.
...
The library currently contains an implementation to generate \gls*{gt} rules for Groove but could also be used for other graph transformation tools in the future.
...
To calculate the average runtime, we used the hyperfine benchmarking tool \cite{peterHyperfine2023} (version 1.18.0), which ran the \gls*{hot}/state space exploration for each \gls*{bpmn} model ten times.
...
We generated 300 \gls*{bpmn} models to test the scalability of our approach.
...
Our \gls*{bpmn} model generation uses the camunda \gls*{bpmn} model API \cite{camundaservicesgmbhCamundaBPMNModel2023}.

The \gls*{bpmn} modeling tools we have used are already struggling with models that have 200 or more blocks.
Thus, we stopped at 300 blocks since we think \gls*{bpmn} models in practice are much smaller, and so we could still run our benchmark in a reasonable time.
\gls*{bpmn} models are developed together by the organization's business and development people.
Consequently, large models are divided into smaller submodels so that humans still understand them.
...
It shows the average runtime of five runs for transforming each \gls*{bpmn} model, calculated by hyperfine \cite{peterHyperfine2023}.
...
The average \gls*{hot} runtimes data fits the linear regression shown in \autoref{fig:hotScalability} well.
This makes sense since the \gls*{hot} algorithm has \textit{linear runtime complexity} since it iterates over all flow nodes of a \gls*{bpmn} model to generate \gls*{gt} rules.
We conclude that the \gls*{hot} is fast enough (around one second or less) for models of reasonable size (50 blocks or less).
...
% Interpretation and takeaways
The increase in the runtime of the state space generation does \textit{not} look linear.
However, models of reasonable size (50 blocks or less) are handled in less than two seconds.
To summarize, using our approach, we can conservatively estimate that these models can be checked (\gls*{hot} followed by a full state space generation) in around three seconds or less.
Plenty of optimization potential, such as parallelization, avoiding costly I/O operations, on-the-fly model checking, partial order reduction, \etc, still exists.
These improvements to the \gls*{hot} or Groove can lead to future improvements in runtime.
...
First, we plan a detailed comparison of our \gls*{hot} approach with approaches that utilize fixed model-independent rules.
