...
First, we improved the usability of the tool in different ways.
We restructured the user interface, previously contained on one page, into three distinct steps.
...
(2) \textbf{Custom Propositions}.

(3) \textbf{Analysis}.
The user is transparently guided through these three steps by the application flow.
Second, the tool now supports defining custom propositions, which can be used for defining temporal logic properties.
% Are downloaded or used directly in model checking
Third, reusable published libraries.
% npm BPMN stuff: token metamodel, token modeler --> provide links in the artifacts
% GT generation library? --> do and provide links in the artifacts
