...
We implement our approach as a user-friendly, open-source web-based tool, the \textit{BPMN Analyzer}, which can be used online without needing installation \cite{timkrauterLMCS2024Artifacts2023}.
...
Parallel gateways represent forks and joins, meaning they delete one token for each incoming sequence flow and add one for each outgoing sequence flow.
...
Checking the properties \textit{Safeness}, \textit{Option to complete} is implemented by checking the CTL properties above using Groove.
