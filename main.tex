...
To make model checking user-friendly, we envision modelers defining atomic propositions based on the extended BPMN syntax, \ie, the concrete syntax introduced in \autoref{fig:bpmnExecutionMetamodel}.
...
The library allows one to create token and process snapshot instances and stores them in the BPMN extension elements, see lines 2-5 in the XML example in \autoref{lst:extension}.
This is the defined way to extend the BPMN metamodel \cite{objectmanagementgroupBusinessProcessModel2013}.
In addition, to define the element a token is located at (see BPMN execution metamodel in \autoref{fig:bpmnExecutionMetamodel}) an association is used (lines 7-8 in \autoref{lst:extension}).

\subsubsection{BPMN token editor}
Our implementation is based on \textit{bpmn-js} \cite{camundaservicesgmbhBpmnjs2023}, which provides a BPMN rendering toolkit and uses the \textit{token-bpmn-moddle} library described in the previous section.

% Add screenshot of editor here probably.
