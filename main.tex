...
A demonstration of our tool is available at \url{https://youtu.be/MxXbNUl6IjE}.
\end{abstract}
...
The HOT creates a GT system, i.e., GT rules and a start graph for a given BPMN model and atomic propositions (see purple-colored elements in \autoref{fig:approach}).
...
(iv) We test the scalability of our approach with 300 synthetically generated BPMN models of increasing size in \autoref{subsec:scalability}. 
...
We will now give an example of predefined general BPMN properties and show how they can be checked using our approach.
...
In the following, we will describe how to implement the \textit{Safeness} and \textit{Option to complete} using CTL, as well as \textit{Proper completion} \textit{No dead activities} by analyzing the GT system's state space.
...
\textit{No dead activities} is implemented by analyzing the GT system's state space to see if each activity has been executed at least once.
The property \textit{Proper Completion} is implemented by checking the GT system's state space for two executions of an end event in the same path \cite{timkrauterLMCS2024Artifacts2023}.
...
An atomic proposition is defined as a process snapshot, together with a token distribution (see \autoref{fig:shippedTwiceAbstractAndGroove} \textbf{(a)}), which we can automatically convert to a graph condition in Groove (see \autoref{fig:shippedTwiceAbstractAndGroove} \textbf{(b)}).
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{images/shippedTwiceProposition.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{images/noShipment.pdf}
