...
In the following, first, we define the BPMN execution metamodel to represent the BPMN state structure, then we explain our formalization of the elements in \autoref{fig:bpmnelementsOverview}.
...
Furthermore, we published our GT rule generation library to the maven central repository (\textit{graph-rule-generation}).
The library currently contains an implementation to generate GT rules for Groove, but it could also be used for other GT tools in the future.

% TODO: Introduce example here.
\autoref{listing:grooveRuleBuilder}

\autoref{fig:grooveRuleBuilderRule} shows the GT rule generated by \autoref{listing:grooveRuleBuilder} in Groove.

\lstinputlisting [label=listing:grooveRuleBuilder, language=Java, numbers=none, caption=Example code to generate a GT rule using the provided GT rule builder]{./listings/abc.txt}
...
Similarly to the groove rule builder in \autoref{listing:grooveRuleBuilder}, there is also a builder for graphs (to create start graphs).
Finally, the library provides a builder for Groove GT systems that uses the graph and GT rule builders.
Groove can then consume the generated GT systems.
