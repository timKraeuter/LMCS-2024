...
%% Do NOT replace the proclamation environments already provided by
%% your own.
...
Although all these elements have been implemented and tested (see \cite{krauterArtifactsLMCS2023}), due to space limitations, we only explain the realization of the elements marked with a green background.
...
\textsf{AllTerminated} is given in \cite{krauterArtifactsLMCS2023}.
...
To make model checking user-friendly, we envision modelers defining atomic propositions based on the extended \gls*{bpmn} syntax, i.e., the concrete syntax introduced in \autoref{fig:typeGraph}.
...
For example, the token distribution shown in \autoref{fig:atomicProposition} defines a process snapshot with two tokens at activity \textit{Ship goods}.
For example, a modeler could use this atomic proposition to check if the activity \textit{Ship goods} is executed twice by creating an appropriate LTL/CTL property.
...
The order handling process in \autoref{fig:atomicProposition} is taken from \cite{ruckerPracticalProcessAutomation2021} but changed to contain the wrong gateway before the \textit{Ship goods} activity.
This leads to shipping goods twice if the process is not corrected before deployment.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\textwidth]{images/shippedTwiceProposition.pdf}
...
Using such a proposition editor, a modeler can work with \gls*{bpmn}, which is well-known to him.
...
% TODO: Tradeoff simplicity vs. expressability
...
Second, we published multiple parts of our application as libraries such that they can be reused seamlessly by other researchers and practitioners \cite{krauterArtifactsLMCS2023}.
...
Finally, we published our \gls*{gt} rule generation library to the maven central repository (\textit{graph-rule-generation}).
...
Third, compared to \cite{krauterFormalizationAnalysisBPMN2023}, we have conducted a more detailed performance investigation of our approach, which is described in the next section.

% TODO: Rewrite an improve upon
...
The experiment was run on Windows 11 (AMD Ryzen 7700X processor, 32 GB RAM) using Groove version 5.8.1 \cite{krauterArtifactsLMCS2023}.
