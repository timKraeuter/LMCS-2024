...
To show the benefits of our approach, we implemented it as an open-source web-based tool. 
\end{abstract}
...
As mentioned in \autoref{sec:introduction}, modelers may use our approach to specify custom properties consisting of atomic propositions and logical operators from CTL.
...
The proposition \textit{noShipment} defines that the activity \textit{Ship goods} should not run.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.65\textwidth]{images/noShipment.pdf}
...
\begin{figure}[ht]
    \centering
    \frame{\includegraphics[width=.8\textwidth]{images/impl_step3.png}}
    \caption{Screenshot of the \textit{analysis step} in the BPMN Analyzer}
    \label{fig:impl_step1}
\end{figure}

The BPMN Analyzer interface is structured into three distinct steps where the user is transparently guided through the modeling and analysis processes by the application flow.
...
\begin{enumerate}
  ...
  \item The \textbf{Custom Propositions} step contains our custom \textit{Token Editor} and is shown in \autoref{fig:impl_step2}.
...
\begin{figure}[ht]
    \centering
    \frame{\includegraphics[width=1\textwidth]{images/impl_step2.png}}
...
\end{figure}
  ...
  \begin{figure}[ht]
      \centering
      \frame{\includegraphics[width=.8\textwidth]{images/impl_step3_analysis.png}}
...
In addition, the Token Editor to define atomic propositions is also published as an npm module (\textit{token-bpmn}) \cite{timkrauterLMCS2024Artifacts2023}.
...
Each token refers to its \textsf{elementID} (see BPMN execution metamodel in \autoref{fig:bpmnExecutionMetamodel}) and \textsf{processSnapshot}.
...
Each \textsf{Token} points to the \textsf{element} it is currently positioned at, which can be an \textsf{Activity} or \textsf{SequenceFlow}, see BPMN metamodel in \autoref{fig:bpmnMetamodel}.
In \autoref{lst:extension}, this is realized using the attribute \textsf{elementID}.

\begin{figure}[ht]
    \centering
    \includegraphics[width=.7\textwidth]{images/bpmn_extension.pdf}
